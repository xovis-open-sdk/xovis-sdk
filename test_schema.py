...
async def main():
    ...
    for t in tools[:3]:
        print(
            f"Name: {t.name}, Desc: {bool(t.description)}, Props have desc: {any('description' in v for v in t.args_schema.model_json_schema().get('properties', {}).values()) if t.args_schema else False}"
        )
