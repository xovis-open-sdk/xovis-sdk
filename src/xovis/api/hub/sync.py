...
class HubSyncer:
    ...
    This utility is used during initial setup to populate the '_local_resources/'
    ...
    async def warmup(self, force: bool = False) -> bool:
        ...
        try:
            async with HubClient(client_id=self.client_id, client_secret=self.client_secret, base_url=self.base_url) as client:
                ...
                state_path = states_dir / "hub_fleet_state.json"
    ...
    async def _fetch_hub_openapi(self, client: HubClient, force: bool):
        ...
        hub_schemas_dir = self.resource_dir / "schemas" / "hub"
        hub_schemas_dir.mkdir(parents=True, exist_ok=True)

        for module_name, endpoint in modules.items():
            target = hub_schemas_dir / f"HUB-{module_name}.json"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    versioned_target = hub_schemas_dir / f"HUB-{module_name}_{version}.json"
